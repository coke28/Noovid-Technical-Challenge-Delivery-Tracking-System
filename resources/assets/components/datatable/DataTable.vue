<template>
  <div>
    ...
    <div class="d-flex flex-stack mb-5">
      ...
      <search-bar-vue v-model="search" @update:modelValue="handleSearchUpdate()"
        @clear-search-data="handleClearSearch()">
      </search-bar-vue>
      <div v-if="parameters.customFormComponent != null">
                ...
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                    <li v-for="tool in parameters.tools" :key="tool.tool_name">
                      <a v-if="tool.tool_name === 'delete'" href="#"
                        @click.prevent="deleteItem(data_row.id, tool.tool_api)" class="dropdown-item">
                        Delete
                      </a>
                      ...
                      <a v-if="tool.tool_name === 'refresh status'" :href="tool.redirect_url" class="dropdown-item">
                        Refresh Status
                      </a>
                    </li>
                  </ul>
    ...
    <div class="d-flex justify-content-end mb-3">
      <div class="form-group d-flex align-items-center">
        ...
        <select class="form-select form-select-sm" id="paginateDisplay" v-model="paginate_display"
          @change="fetchData()">
          <option v-for="(paginate_display, index) in parameters.page_display" :key="index" :value="paginate_display">{{
            paginate_display }}
          </option>
        </select>
      </div>
...
export default {
  ...
  mounted() {
    // Fetch initial data
    this.fetchData(1);
    // Initialize Laravel Echo and subscribe to the channel
    const channel = Echo.channel('public-delivery-status.1');
    // Listen for events on the channel
    channel.listen('UpdateDeliveryStatus', (data) => {
